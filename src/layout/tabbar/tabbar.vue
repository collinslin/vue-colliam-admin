<template>
  ...
  <transition name="el-zoom-in-top">
    <DropdownNav :left="left" v-show="isShow" @click-tabbar-menu="chooseTabbarMenu"></DropdownNav>
  </transition>
</template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const cachedTabsStr = sessionStorage.getItem(import.meta.env.VITE_APP_TABBAR)
      ...
      const findTargetIndex = (path: string): number => {
        const index = tabbarData.value!.findIndex(item => item.path == path) as number
        return index
      }
      ...
      const typeList: { [key: string]: (targetPath: string) => void } = {
        ...
        closeAll: targetPath => {
          ...
          const target = tabbarData.value!.find((item, index) => targetIndex == index)
